...
import { useDispatch, useSelector } from "react-redux";
import { setAnalysis } from "../redux/analysisSlice";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faUpload, faRobot } from "@fortawesome/free-solid-svg-icons";
...
const ResumeAnalyzer = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (uploadRes.ok) {
        dispatch(setAnalysis(uploadData.analysis));
      } else {
        alert(uploadData.error || "Something went wrong");
      }
    } catch (err) {
  ...
  return (
    <div className="bg-[#0f172a] min-h-screen p-6 sm:p-12 text-white">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl sm:text-4xl font-bold mb-8 text-center text-blue-400">
          Resume Analyzer
        </h1>

        <div className="bg-[#1e293b] p-6 sm:p-8 rounded-2xl shadow-lg border border-[#334155] mb-10">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <input
              type="file"
              accept=".pdf"
              onChange={handleFileChange}
              className="text-sm sm:text-base p-2 bg-[#0f172a] border border-[#334155] rounded-lg w-full sm:w-auto"
            />
            <button
                ...
                .map((section, index, arr) => {
                  if (index % 2 === 0) {
                    ...
                    return (
                      <h3
                        key={index}
                        className="text-2xl font-bold text-white mt-6 mb-3 border-b border-gray-600 pb-2"
                      >
                        {section.trim()}
                      </h3>
                    );
                  ...
                  return (
                    <div key={index} className="space-y-4">
                      {section
                        .split(/\n(?=\d+\.)/) // Split at numbered list
                        .filter(Boolean)
                        .map((line, i) => (
                          <div
                            key={i}
                            className="bg-[#2d3748] p-4 rounded-xl border border-[#3b4758] shadow-inner"
                          >
